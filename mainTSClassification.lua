...
if options.gcnNormalize then
   print ' - Perform Globam Contrast Normalization (GCN) on input data'
...
if options.zcaWhitening then
   ...
   require 'mainPreprocess'
   local means, P = zca_whiten_fit(torch.cat(unSets["TRAIN"].data, unSets["VALID"].data, 1));
   ...
   for k, v in pairs(setList) do
    local means, P = zca_whiten_fit(torch.cat(sets[v]["TRAIN"].data, sets[v]["VALID"].data, 1));
  ...
  if curModel.pretrain then
    ...
    for l = 1,structure.nLayers do
      ...
      if (not options.adaptiveLearning) then 
        if torch.type(unsupData.data) ~= 'table' then configureOptimizer(options, unsupData.data:size(2)) else configureOptimizer(options, #unsupData.data); end   
      end
      epoch = 0;
      prevValid = 5e20;
      ...
      if torch.type(forwardedValid.data) ~= 'table' then forwardedValid.data = forwardedValid.data:clone() else
        for i = 1,#forwardedValid.data do forwardedValid.data[i] = forwardedValid.data[i]:clone() end
      end
      -- Replace previous set
